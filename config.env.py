...
LDAP_BIND_DN = env.get(
    "CONDITIONAL_LDAP_BIND_DN", "cn=conditional,ou=Apps,dc=csh,dc=rit,dc=edu"
)
...
# OIDC Config
OIDC_ISSUER = env.get(
    "CONDITIONAL_OIDC_ISSUER", "https://sso.csh.rit.edu/auth/realms/csh"
)
OIDC_CLIENT_CONFIG = {
    "client_id": env.get("CONDITIONAL_OIDC_CLIENT_ID", "conditional"),
    "client_secret": env.get("CONDITIONAL_OIDC_CLIENT_SECRET", ""),
    "post_logout_redirect_uris": [
        env.get("CONDITIONAL_OIDC_CLIENT_LOGOUT", "http://0.0.0.0:6969/logout")
    ],
}
FROSH_OIDC_ISSUER = env.get(
    "CONDITIONAL_FROSH_OIDC_ISSUER", "https://sso.csh.rit.edu/auth/realms/csh"
)
